...
const Dashboard: React.FC = () => {
  const [data, setData] = useState<Certificazione[]>([]);
  ...
  const chartData = Object.entries(certificationsByType).map(([tipo, count]) => ({
    tipoCertificazione: tipo,
    count
  }));
  ...
  return (
    <div className="p-6 text-white bg-gray-900 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-green-400">Dashboard Certificazioni</h1>

      {/* KPI */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-gray-800 p-4 rounded shadow text-center">
          <h3 className="text-lg font-semibold">Totale Certificazioni</h3>
      ...
      <div className="mb-6 max-w-md">
        <label className="block text-sm text-gray-300 mb-2">Filtra per Paesi:</label>
          ...
          <div className="relative mt-1">
            <Listbox.Button className="relative w-full cursor-default rounded-lg bg-white text-black py-2 pl-3 pr-10 text-left shadow-md sm:text-sm">
              <span className="block truncate">
                {selectedCountries.length > 0 ? selectedCountries.join(', ') : 'Seleziona uno o più paesi'}
              </span>
            </Listbox.Button>
            <Listbox.Options className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white text-black py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 sm:text-sm">
              {[allOption, ...uniqueCountries].map((country) => (
                <Listbox.Option key={country} value={country} className="cursor-pointer select-none py-2 pl-10 pr-4">
                  {({ selected }) => (
                    <>
                      <span className={`block truncate ${selected ? 'font-medium' : 'font-normal'}`}>
                        {country}
                      </span>
                      {selected ? <CheckIcon className="absolute left-2 h-5 w-5 text-green-600" aria-hidden="true" /> : null}
                    </>
                  )}
                </Listbox.Option>
              ))}
      ...
      {/* ANNI */}
      <div className="mb-4">
        <h3 className="text-gray-300 mb-1">Filtra per Anno:</h3>
      ...
      {/* MESI */}
      <div className="mb-6">
        <h3 className="text-gray-300 mb-1">Filtra per Mese:</h3>
        <div className="flex flex-wrap gap-2">
      ...
      <div className="bg-gray-800 p-4 rounded shadow mb-8">
        <h2 className="text-xl mb-4 font-semibold">Certificazioni per Tipo</h2>
        ...
        <table className="min-w-full divide-y divide-gray-700">
          <thead className="bg-gray-800">
            <tr>
              ...
              <th className="px-4 py-2 text-left">Tipo Certificazione</th>
